...
public class ChairsUtils {
    ...
    protected static List<Object> calculateSitLocation(Player player, Block block) {
        ...
        // Permettra de récupérer le nouveau calcul de la localisation pour que le Joueur s'asseye avec la raison du retour.
        List<Object> calculLocation = new ArrayList<>();
        ...
        double x = 0.0; // Permet de récupérer la coordonnée 'X' éxacte à envoyé
        double z = 0.0; // Permet de récupérer la coordonnée 'Y' éxacte à envoyé
        ...
        if(distance <= 3.5) {
            ...
            if(blockdata instanceof Stairs stairs) {
                ...
                // ⬇️ Essaie de définir le "Yaw" et les coordonnées X ou Z en fonction de la rotation exacte de la face de l'escalier ⬇️ //
                switch(ascendingFacing.getOppositeFace()) {

                    case NORTH -> {yaw = 180; z = -0.2; } // Rotation vers le nord : Enlève "0.2" à la coordonnée 'Z' et définit "180" à la rotation 'Yaw'
                    case EAST -> { yaw = -90; x = 0.2; } // Rotation vers le l'est : Ajoute "0.2" à la coordonnée 'X' et définit "-90" à la rotation 'Yaw'
                    case SOUTH -> { yaw = 0; z = 0.2; } // Rotation vers le sud : Ajoute "0.2" à la coordonnée 'Z' et définit "0" à la rotation 'Yaw'
                    case WEST -> { yaw = 90; x = -0.2; } // Rotation vers l'ouest : Enlève "0.2" à la coordonnée 'X' et définit "90" à la rotation 'Yaw'
                }
                // ⬆️ Essaie de définir le "Yaw" et les coordonnées X ou Z en fonction de la rotation exacte de la face de l'escalier ⬆️ //
            ...
            // Sinon, on vérifie si le bloc est une dalle, alors on effectue le calcul pour la dalle
        ...
        if(calculLocation.get(0) instanceof Location loc) {
         ...
         loc.setPitch(pitch); //Définit la rotation "Pitch" de la Localisation récupérée par celui du Joueur
            ...
            calculLocation.set(0, loc); // Réattribue la Localisation récupérée avec les modifications effectuée
        }
        // ⬆️ Si la liste 'calculLocation' contient en première position une Localisation, on change alors sa rotation ⬆️ //
    ...
    public static void standup(Player player, Entity entityTarget) {
        ...
        if(entityTarget.getPassengers().contains(player)) {
            ...
            entityTarget.remove(); // Supprime l'entité cible
            ...
            // ⬇️ Essaie de définir les coordonnées X ou Z en fonction de la rotation exacte de la face demandée ⬇️ //
            switch(rot(face).getOppositeFace()) {

                case NORTH -> x = -0.75;  // Rotation vers le nord : Enlève "0.75" à la coordonnée 'X'
                case EAST -> z = -0.75;  // Rotation vers le sud : Enlève "0.75" à la coordonnée 'Z'
                case SOUTH -> x = 0.75; // Rotation vers le l'est : Ajoute "0.75" à la coordonnée 'X'
                case WEST -> z = 0.75; // Rotation vers l'ouest : Ajoute "0.5" à la coordonnée 'Z'
            }
            // ⬆️ Essaie de définir les coordonnées X ou Z en fonction de la face demandée ⬆️ //

            // Récupère la nouvelle localisation auquel le joueur sera téléporté (évite les collisions)
            ...
            player.sendMessage(ChatFormatting.GRAY.toString() + ChatFormatting.ITALIC.toString() + "Vous n'êtes plus assis."); // Envoie un message au Joueur
        }
        // On vérifie si l'entité cible, a le Joueur assis sur lui, alors on continue //
    }

    /**
     * Le joueur s'assied sur une Entité {@link ArmorStand}, s'il a la permission requise et si c'est un escalier ou une dalle
     *
    ...
    public static void sitDown(Player player, Block blockTarget) {
        ...
        if(blockdata instanceof Stairs || blockdata instanceof Slab) {
            ...
            // Sinon, vérifie si le type du block est une dalle et on vérifie si ce n'est pas deux dalle double, ou une dalle du bas, sinon on annule le code
                ...
                if(slab.getType() == Slab.Type.DOUBLE /*|| slab.getType() != Slab.Type.BOTTOM*/) return;
            }

                            /* --------------------------------------------------------------- */

            //Vérifie si le joueur à la permission requise, sinon rien ne se passe
            if(player.hasPermission("evhohub.chairs")) {
                ...
                if(SitLocation.get(0) instanceof String message) {
                    ...
                    player.sendMessage(ChatFormatting.RED.toString() + ChatFormatting.ITALIC.toString() + message);
                    return;
                }
                // Si la liste 'SitLocation' contient uniquement un message, rien ne se passe, mais on envoie le message au joueur. //
                ...
                for(Entity entity : entities) {
                    ...
                    if(entity instanceof Player p) hasOnlyPlayer = p == player;
                }
                // Vérifie s'il n'y a aucune Entité dans la 'hitbox' du bloc cible ou s'il y a que le joueur que le Joueur en question //

                // On annule s'il y a des entités dans la 'hitbox' du bloc cible ou si la 'hitbox' du bloc ne contient pas que le Joueur en question
                if(!(entities.isEmpty() || hasOnlyPlayer)) {

                    player.sendMessage(ChatFormatting.RED.toString() + ChatFormatting.ITALIC.toString() + "Quelque chose vous empêche de vous asseoir :(.");
                    return;
                }

                                     /* ------------------------------------------------------- */

                ArmorStand armorStand = generateArmorStand(player, SitLocation, level); // Génère le porte armure à monter
                level.addFreshEntity(armorStand, CreatureSpawnEvent.SpawnReason.CUSTOM); // Fait apparaître le Porte Armure pour permettre au Joueur de le monter
                ((CraftPlayer)player).getHandle().startRiding(armorStand, true); // Le Joueur va monter le Porte Armure

                                    /* ------------------------------------------------------- */

                player.sendMessage(ChatFormatting.GRAY.toString() + ChatFormatting.ITALIC.toString() + SitLocation.get(1));  // Envoie le message d'information au Joueur
            }
        }
		// Vérifie si le type du block est un escalier ou une dalle //
    }
    ...
    /**
     * Créer une entité de 'porte-armure' pour permettre à un joueur de s'asseoir.
     *
     * @param player Le joueur pour lequel créer le 'porte-armure'
     * @param SitLocation Une liste contenant la localisation calculée pour que le joueur puisse s'asseoir
     * @param level Le monde du serveur sur lequel créer le 'porte-armure'
     *
     * @return Une entité de 'porte-armure' configuré pour permettre au joueur de s'asseoir
     *
     * @throws NullPointerException Si l'une des entrées passées est null
     */
    @NotNull
    private static ArmorStand generateArmorStand(Player player, List<Object> SitLocation, ServerLevel level) {
